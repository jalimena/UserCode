{
  ...
  TFile f2 ("/home/alimena/histos/Triggers_csmt/histosHltAnalyzer_mchamp500_wotriggers_csmt_L1MuOpenSeeds.root");
  ...
  //TFile f2 ("/home/alimena/histos/Triggers_csmt/histosHltAnalyzer_mchamp500_csmt_L1MuOpenSeeds.root");
  ...
  */
  ...
  TString outfile_path0 = "../txtfiles/HltPlotIntegral_plot0.txt"; 
  TString outfile_path1 = "../txtfiles/HltPlotIntegral_plot1.txt"; 
  ofstream outfile0(outfile_path0);
  ofstream outfile1(outfile_path1);

  Double_t integral0[200];
  Double_t integral1[200];
  Double_t error0[200];
  Double_t error1[200];
  for(int i=1; i<=200; i++){
    integral0[i-1] = HAll1D[0][0]->IntegralAndError(i,201,error0[i-1]);
    outfile0<<integral0[i-1]<<" "<<error0[i-1]<<endl;
  }
  for(int i=1; i<=200; i++){
    integral1[i-1] = HAll1D[1][0]->IntegralAndError(i,201,error1[i-1]);
    outfile1<<integral1[i-1]<<" "<<error1[i-1]<<endl;
  }
  ...
  */
  ...
  //char* LegFile[NumFiles] = {"No Trigger Required","Original (without CS or MT)","With L1_SingleMuOpen and CS and MT"};
  //char* LegFile[NumFiles] = {"Original (without CS or MT)","With L1_SingleMuOpen and CS and MT"};
  //char* LegFile[NumFiles] = {"Generator","2012 Configuration","L1 and L2 Improvements for 2015"};
  char* LegFile[NumFiles] = {"Configuration in 2012","L1 and L2 Improvements for 2015"};
