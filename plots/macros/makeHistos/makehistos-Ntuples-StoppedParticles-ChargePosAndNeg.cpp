{
  ...
  TFile f1 ("/home/alimena/histos/mchamp500_reducedNtuples_724/wBxCut_woCavCut_woGenMuCut_woSACut_woGenMuMatchedCut_woTriggerCut_woPtCut_woChaCut_woEtaCut_woUpperCut_woRpcCut_woDisStCut_woRpcBxCut__woDtHitsCut_woCscHitsCut_woDtInvBetaCut_woTimeInOutCut_woOppEtaCut_woOppPhiCut_woTightPhi_woCosEnrich.root");

  //cout<<"Loaded file: "<<file<<endl;

  //info for each 1D plot
  const int Num1DHists = 5;
  char* plot1DPos[Num1DHists] = {
    "mcStoppedParticleXPos_", //0
    "mcStoppedParticleYPos_", //1
    "mcStoppedParticleZPos_", //2
    "mcStoppedParticleRPos_", //3
    "mcStoppedParticle0Time_" //4
    /*
      "mcStoppedParticleXPos2Stopped_", //4
    "mcStoppedParticleYPos2Stopped_", //5
    "mcStoppedParticleZPos2Stopped_", //6
    "mcStoppedParticleRPos2Stopped_" //7
			    */
  };
  char* plot1DNeg[Num1DHists] = {
    "mcStoppedParticleXNeg_", //0
    "mcStoppedParticleYNeg_", //1
    "mcStoppedParticleZNeg_", //2
    "mcStoppedParticleRNeg_", //3
    "mcStoppedParticle1Time_" //4
    /*
    "mcStoppedParticleXNeg2Stopped_", //3
    "mcStoppedParticleYNeg2Stopped_", //4
    "mcStoppedParticleZNeg2Stopped_" //5
    "mcStoppedParticleRNeg2Stopped_" //5
			    */
  };

  char* xtitle1D[Num1DHists] = {
    "Stopped Particle X [cm]",
    "Stopped Particle Y [cm]",
    "Stopped Particle Z [cm]",
    "Stopped Particle R [cm]",
    "Stopped Particle Stopping Time [ns]"
    /*,
    "Stopped Particle X [cm]",
    "Stopped Particle Y [cm]",
    "Stopped Particle Z [cm]",
  ...
  */
  ...
  for (int i=0; i<Num1DHists; i++){
    ...
    sprintf(Ctitle1D,"%s",plot1DPos[i]);
  ...
  */
  ...
  char oldH1DnamePos[100], oldH1DnameNeg[100];
  for(int i=0; i<Num1DHists; i++){
    sprintf(oldH1DnamePos,"%shist",plot1DPos[i]);
    sprintf(oldH1DnameNeg,"%shist",plot1DNeg[i]);
    HAll1D[0][i] = (TH1D*) f1.Get(oldH1DnamePos);
    HAll1D[1][i] = (TH1D*) f1.Get(oldH1DnameNeg);
  ...
  */
  ...
  //char* LegFile[NumFiles] = {"Positive Mchamp","Negative Mchamp"};
  char* LegFile[NumFiles] = {"1st Mchamp","2nd Mchamp"};
  ...
  */
  ...
  for (int a=0; a<NumFiles; a++){
    //HAll1D[a][3]->GetYaxis()->SetRangeUser(0,10);
    //HAll1D[a][4]->GetYaxis()->SetRangeUser(0,12);
    //HAll1D[a][5]->GetYaxis()->SetRangeUser(0,9);
    /*
