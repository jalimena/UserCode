{
  ...
  */
  ...
  TFile f2("/home/alimena/histos/test_timing_allphi_cosmics_pt20_eta1.root");
  TFile f3("/home/alimena/histos/test_timing_stop600_neutralino417_pt20_eta1.root");
  */

  /*
  TFile f1("/home/alimena/histos/test_timing_zmumu_pt20_eta1_DTChambers2.root");
  TFile f2("/home/alimena/histos/test_timing_allphi_cosmics_pt20_eta1_DTChambers2.root");
  ...
  */
  ...
  TFile f2("/home/alimena/histos/test_timing_allphi_cosmics_pt20_eta1_DTChambers2_upper.root");
  TFile f3("/home/alimena/histos/test_timing_stop600_neutralino417_pt20_eta1_DTChambers2_upper.root");
  ...
  */
  ...
  const int Num2DHists = 7;
  char* plot2D[Num2DHists] = {"hi_dttime_ibt_pt","hi_dttime_ibt_ibt_err","sta_phi_dttime_invbetaLS_hist","dttime_invbetaLS_chi2Dof_hist","dttime_invbetaLS_fib","dttime_localT0_hitChisq","dttime_localT0_hitChisq_LowInversebetaLS"};
  char* xtitle2D[Num2DHists] = {"SA p_{T} [GeV]","DT #beta^{-1}","SA #phi","DT #beta^{-1}","DT #beta^{-1}","Hit T0 [ns]","Hit T0 [ns]"};
  char* ytitle2D[Num2DHists] = {"DT #beta^{-1}","Error of DT #beta^{-1}","DT #beta^{-1}","#chi^{2}/dof","Free #beta^{-1}","Hit #chi^{2}","Hit #chi^{2}"};
  ...
  for (int i=2.*Num2DHists; i<1.0*NumFiles*Num2DHists; i++){
    sprintf(Cname2D,"canvases2D[%i]",i);
  ...
  int color[NumFiles] = {1,2,4};
  int style[NumFiles] = {1,2,3};
  int marker[NumFiles] = {20,21,22};
  char axistitles[200];
  ...
  for (int a=0; a<NumFiles; a++){
    ...
    HAll1D[a][14]->GetXaxis()->SetRangeUser(-5,5);
    ...
    HAll2D[a][4]->GetXaxis()->SetRangeUser(-5,5);
    HAll2D[a][4]->GetYaxis()->SetRangeUser(-5,5);

    HAll2D[a][5]->GetXaxis()->SetRangeUser(-30,30);
    HAll2D[a][5]->GetYaxis()->SetRangeUser(0,15);

    HAll2D[a][6]->GetXaxis()->SetRangeUser(-30,30);
    HAll2D[a][6]->GetYaxis()->SetRangeUser(0,15);
  ...
  TPaveLabel *Leg2a = new TPaveLabel(.19,.92,.39,.99,"Z#rightarrow#mu#mu Data","NDC");
  Leg2a->SetBorderSize(0);
  Leg2a->SetTextSize(0.7);
  Leg2a->SetFillColor(0);

  TPaveLabel *Leg2b = new TPaveLabel(.19,.92,.39,.99,"All-Phi Cosmic Data","NDC");
  Leg2b->SetBorderSize(0);
  Leg2b->SetTextSize(0.7);
  Leg2b->SetFillColor(0);

  TPaveLabel *Leg2c = new TPaveLabel(.19,.92,.39,.99,"#tilde{t_{1}} 600 GeV","NDC");
  ...
  Double_t scalesignal[Num1DHists];

  for(int i=0; i<Num1DHists; i++){
    scalecosmics[i] = (1.0*(Double_t)HAll1D[0][i]->GetSumOfWeights())/((Double_t)HAll1D[1][i]->GetSumOfWeights());
    scalesignal[i] = (1.0*(Double_t)HAll1D[0][i]->GetSumOfWeights())/((Double_t)HAll1D[2][i]->GetSumOfWeights());

    HAll1D[1][i]->Scale(scalecosmics[i]);
    HAll1D[2][i]->Scale(scalesignal[i]);
  }
  ...
  for (int a=0; a<NumFiles; a++){
    for(int i=0; i<Num1DHists; i++){
      ...
      else if(i==9) HAll1D[a][i]->GetYaxis()->SetRangeUser(0,500);
  ...
  for(int i=0; i<Num1DHists; i++){
    ...
    //sprintf(canvasesEPS,"../plots/timing/DT/zmumu_cosmic_signal/%s.eps",plot1D[i]);
    //sprintf(canvasesEPS,"../plots/timing/DT/pt20_eta1_DTchambers2/zmumu_cosmic_signal/%s.eps",plot1D[i]);
    sprintf(canvasesEPS,"../plots/timing/DT/pt20_eta1_DTchambers2_upper/zmumu_cosmic_signal/%s.eps",plot1D[i]);
  ...
  for(int i=0; i<Num2DHists; i++){
    ...
    Leg2a->Draw();
    text1->Draw();
    //sprintf(canvasesEPS2D,"../plots/timing/DT/zmumu_cosmic_signal/%szmumu.eps",plot2D[i]);
    //sprintf(canvasesEPS2D,"../plots/timing/DT/pt20_eta1_DTchambers2/zmumu_cosmic_signal/%szmumu.eps",plot2D[i]);
    sprintf(canvasesEPS2D,"../plots/timing/DT/pt20_eta1_DTchambers2_upper/zmumu_cosmic_signal/%szmumu.eps",plot2D[i]);
  ...
  for(int i=Num2DHists; i<2*Num2DHists; i++){
    ...
    Leg2b->Draw();
    text1->Draw();
    //sprintf(canvasesEPS2D,"../plots/timing/DT/zmumu_cosmic_signal/%sstop600_neutralino417.eps",plot2D[i-2*Num2DHists]);
    //sprintf(canvasesEPS2D,"../plots/timing/DT/pt20_eta1_DTchambers2/zmumu_cosmic_signal/%sallphi_cosmics.eps",plot2D[i-Num2DHists]);
    sprintf(canvasesEPS2D,"../plots/timing/DT/pt20_eta1_DTchambers2_upper/zmumu_cosmic_signal/%sallphi_cosmics.eps",plot2D[i-Num2DHists]);
    if (save_plots) canvases2D[i]->SaveAs(canvasesEPS2D);
  }
  for(int i=2*Num2DHists; i<3*Num2DHists; i++){
    ...
    text1->Draw();
    //sprintf(canvasesEPS2D,"../plots/timing/DT/zmumu_cosmic_signal/%sstop600_neutralino417.eps",plot2D[i-2*Num2DHists]);
    //sprintf(canvasesEPS2D,"../plots/timing/DT/pt20_eta1_DTchambers2/zmumu_cosmic_signal/%sstop600_neutralino417.eps",plot2D[i-2*Num2DHists]);
    sprintf(canvasesEPS2D,"../plots/timing/DT/pt20_eta1_DTchambers2_upper/zmumu_cosmic_signal/%sstop600_neutralino417.eps",plot2D[i-2*Num2DHists]);
    if (save_plots) canvases2D[i]->SaveAs(canvasesEPS2D);
  }
  /*
  for(int i=2*Num2DHists; i<3*Num2DHists; i++){
