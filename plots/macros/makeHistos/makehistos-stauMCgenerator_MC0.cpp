{
  ...
  char* plot[NumHists] = {"_0MCnum","_0MCpid","_0MCstatus","_0MCpt","_0MCeta","_0MCphi","_0MCvtxX","_0MCvtxY","_0MCvtxZ"};
  char* xtitle[NumHists] = {"Particle Number if p_{T}, #eta, or #phi == 0.","Particle ID Numbers if p_{T}, #eta, or #phi == 0. (low)","MC status if p_{T}, #eta, or #phi == 0.","Generator p_{T} [GeV] if p_{T}, #eta, or #phi == 0.","Generator #eta if p_{T}, #eta, or #phi == 0.","Generator #phi if p_{T}, #eta, or #phi == 0.","Generator x-coordinate of vertex [cm] if p_{T}, #eta, or #phi == 0.","Generator y-coordinate of vertex [cm] if p_{T}, #eta, or #phi == 0.","Generator z-coordinate of vertex [cm] if p_{T}, #eta, or #phi == 0."};
  ...
  for(int a=0; a<NumFiles; a++){
    for(int i=0; i<NumHists; i++){
      ...
      //cout<<"xtitle[i] is: "<<xtitle[i]<<endl;
    }
  }
  ...
  for (int a=0; a<NumFiles; a++){
    ...
    HAll[a][1]->GetXaxis()->SetRangeUser(-20,120);
    //HAll[a][1]->GetXaxis()->SetRangeUser(1000000,1000030);
    HAll[a][3]->GetXaxis()->SetRangeUser(0,100);
    HAll[a][4]->GetXaxis()->SetRangeUser(-4.5,4.5);
    HAll[a][5]->GetXaxis()->SetRangeUser(-3.5,3.5);
    HAll[a][6]->GetXaxis()->SetRangeUser(-3000,3000);
    HAll[a][7]->GetXaxis()->SetRangeUser(-3000,3000);
    HAll[a][8]->GetXaxis()->SetRangeUser(-6000,6000);
  }    
  ...
  for(int i=0; i<NumHists; i++){
    ...
    sprintf(canvasesEPS,"../plots/stauMCgenerator/MC0/%s.eps",plot[i]);
