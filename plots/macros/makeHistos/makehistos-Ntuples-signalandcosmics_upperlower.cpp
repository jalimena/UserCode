{
  ...
  */
  ...
  TFile f5("/home/alimena/histos/NoBPTX_cosmics_allphi_wBxCut_wCavernCut_wGeneratorMuonCut_wSACut_woGenMuonMatchedCut_wTriggerCut_wPtCut_wChaCut_wEtaCut_InvtwUpperCut_wRpcCut_woRpcBxCut__wDtHitsCut_woDtInvBetaCut_woTimeInOutCut_woTightPhi.root");
  //cout<<"Loaded file: "<<file<<endl;

  //info for each 1D plot
  //const int Num1DHists = 95;a
  //const int Num1DHists = 45;
  const int Num1DHists = 17;
  ...
  */
  ...
  char* LegFile[NumFiles] = {"H^{++} 100 GeV","H^{++} 500 GeV","H^{++} 1000 GeV","Cosmic All-Phi Data, #phi>0", "Cosmic All-Phi Data, #phi<0"};
  char LegEntry[100];
  ...
  */
  ...
  for(int i=0; i<Num1DHists; i++){
    scalef1[i] = (1.0*(Double_t)HAll1D[3][i]->GetSumOfWeights() + 1.0*(Double_t)HAll1D[4][i]->GetSumOfWeights())/((Double_t)HAll1D[0][i]->GetSumOfWeights());
    scalef2[i] = (1.0*(Double_t)HAll1D[3][i]->GetSumOfWeights() + 1.0*(Double_t)HAll1D[4][i]->GetSumOfWeights())/((Double_t)HAll1D[1][i]->GetSumOfWeights());
    scalef3[i] = (1.0*(Double_t)HAll1D[3][i]->GetSumOfWeights() + 1.0*(Double_t)HAll1D[4][i]->GetSumOfWeights())/((Double_t)HAll1D[2][i]->GetSumOfWeights());

    HAll1D[0][i]->Scale(scalef1[i]);
  ...
  for (int a=0; a<NumFiles; a++){
    for(int i=0; i<Num1DHists; i++){
      ...
      else HAll1D[a][i]->GetYaxis()->SetRangeUser(.1,10000); //for stops, mchamps
  ...
  //for(int i=0; i<17; i++){
    ...
    */
    ...
    sprintf(canvasesEPS,"../plots/StoppedHSCPMuon/RecoData/cosmics_allphi_mchamps_upper_lower/%s.eps",plot1D[i]);
