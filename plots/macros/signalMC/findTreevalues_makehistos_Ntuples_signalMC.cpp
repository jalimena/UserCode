...
int main(int argc, char* argv[]){
  ...
  TH1D* mcLL_N_hist = new TH1D("mcLL_N_hist","Number of Gen LL Particles",10,0,10);
  TH1D* mcLLStatus_hist = new TH1D("mcLLStatus_hist","Gen LL Particle Status",10,0,10);
  TH1D* mcLLPt_hist = new TH1D("mcLLPt_hist","Gen LL Particle p_{T}",200,0,200);
  TH1D* mcLLEta_hist = new TH1D("mcLLEta_hist","Gen LL Particle #eta",120,-6,6);
  TH1D* mcLLPhi_hist = new TH1D("mcLLPhi_hist","Gen LL Particle #phi",64,-6.2832,6.2832);
  TH1D* mcLLCharge_hist = new TH1D("mcLLCharge_hist","Gen LL Particle Charge",6,-3,3);
  TH1D* mcLLMass_hist = new TH1D("mcLLMass_hist","Gen LL Particle Mass",40,0,0.2);
  TH1D* mcLLNDaughters_hist = new TH1D("mcLLNDaughters_hist","Number of Daughters of Gen LL Particles",10,0,10);
  TH1D* mcLLDaughterId_hist = new TH1D("mcLLDaughterId_hist","Daughter Id of Gen LL Particles",50,-25,25);
  ...
  int mb_count_ = 0;
  int me_count_ = 0;
  int other_detector_count_ = 0;
  int cavern_count_ = 0;
  int detector_count_ = 0;
  
  int pass_bx_cut = 0;
  int pass_cavern_cut = 0;
  int pass_Ntops_cut = 0;
  int pass_Nstops_cut = 0;
  ...
  for (Int_t i=0; i<nentries; i++) {
	...
	//did status1 muon come from a tau?
	if(TMath::Abs(events->mcMuonMotherId[j])==15){
	  status1muonFromTau = true;
	  //check that at least one tau came from W, or from CMshower from W
	  //for (UInt_t k=0; k<events->mcTau_N; k++) {
	  //if(TMath::Abs(events->mcTauMotherId[k])==24) status1muonFromTau = true;
	  //if(TMath::Abs(events->mcTauMotherId[k])==94){
	  //for (UInt_t l=0; l<events->mcCMshower_N; l++) {
	  //if(TMath::Abs(events->mcCMshowerMotherId[l])==24) status1muonFromTau = true;			  
	  // }
	  //}
	  //}
	}
	...
	if(TMath::Abs(events->mcMuonMotherId[j])!=13 && TMath::Abs(events->mcMuonMotherId[j])!=15 && TMath::Abs(events->mcMuonMotherId[j])!=24){
	  if(TMath::Abs(events->mcMuonMotherId[j])!=5332 && TMath::Abs(events->mcMuonMotherId[j])!=5232 && TMath::Abs(events->mcMuonMotherId[j])!=5132 && TMath::Abs(events->mcMuonMotherId[j])!=5122 && TMath::Abs(events->mcMuonMotherId[j])!=4332 && TMath::Abs(events->mcMuonMotherId[j])!=4232 && TMath::Abs(events->mcMuonMotherId[j])!=4132 && TMath::Abs(events->mcMuonMotherId[j])!=4122 && TMath::Abs(events->mcMuonMotherId[j])!=541 && TMath::Abs(events->mcMuonMotherId[j])!=531 && TMath::Abs(events->mcMuonMotherId[j])!=521 && TMath::Abs(events->mcMuonMotherId[j])!=511 && TMath::Abs(events->mcMuonMotherId[j])!=443 && TMath::Abs(events->mcMuonMotherId[j])!=431 && TMath::Abs(events->mcMuonMotherId[j])!=421 && TMath::Abs(events->mcMuonMotherId[j])!=411 && TMath::Abs(events->mcMuonMotherId[j])!=333 && TMath::Abs(events->mcMuonMotherId[j])!=221 && TMath::Abs(events->mcMuonMotherId[j])!=113){
	...
	if(TMath::Abs(events->mcMuonMotherId[j])==15 && !status1muonFromTau) cout<<"mother of stable muon is tau but didn't work for event "<<events->id<<endl;
	...
	if( (do_generatorMuon_cut && events->mcTop_N>0) || (!do_generatorMuon_cut)){
	  pass_Ntops_cut++;
	  if( (do_generatorMuon_cut && events->mcStop_N>0) || (!do_generatorMuon_cut)){
	    pass_Nstops_cut++;
	    if( (do_generatorMuon_cut && WFromTop)  || (!do_generatorMuon_cut)){
	      pass_WFromTop_cut++;
		...
		if( (do_generatorMuon_cut && status1muon) || (!do_generatorMuon_cut)){
		  ...
		  if( (do_generatorMuon_cut && (status1muonFromMuon || status1muonFromW)) || (!do_generatorMuon_cut)){
		    ...
		    //for (UInt_t j=0; j<events->mcTop_N; j++) {
		    mcTopStatus_hist->Fill(events->mcTopStatus[0],1.0);
		    mcTopPt_hist->Fill(events->mcTopPt[0],1.0);
		    mcTopEta_hist->Fill(events->mcTopEta[0],1.0);
		    mcTopPhi_hist->Fill(events->mcTopPhi[0],1.0);
		    mcTopCharge_hist->Fill(events->mcTopCharge[0],1.0);
		    mcTopMass_hist->Fill(events->mcTopMass[0],1.0);
		    mcTopNDaughters_hist->Fill(events->mcTopNDaughters[0],1.0);
		    for (UInt_t k=0; k<events->mcTopNDaughters[0]; k++) {
		      //cout<<"For daughter # "<<k<<", top daughter ID is: "<<events->mcTopDaughterId.at(0).at(k)<<endl;
		      mcTopDaughterId_hist->Fill(events->mcTopDaughterId.at(0).at(k),1.0);
		      if(TMath::Abs(events->mcTopDaughterId.at(0).at(k))==24) num_WFromTop++;
		      if(TMath::Abs(events->mcTopDaughterId.at(0).at(k))==21) num_gFromTop++;
		      if(TMath::Abs(events->mcTopDaughterId.at(0).at(k))==6) num_tFromTop++;
		      if(TMath::Abs(events->mcTopDaughterId.at(0).at(k))==5) num_bFromTop++;
		      if(TMath::Abs(events->mcTopDaughterId.at(0).at(k))!=5 && TMath::Abs(events->mcTopDaughterId.at(0).at(k))!=6  && TMath::Abs(events->mcTopDaughterId.at(0).at(k))!=21  && TMath::Abs(events->mcTopDaughterId.at(0).at(k))!=24)  num_otherFromTop++;
		    }
		    //}
		    
		    mcW_N_hist->Fill(events->mcW_N,1.0);
		    //for (UInt_t j=0; j<events->mcW_N; j++) {
		    mcWStatus_hist->Fill(events->mcWStatus[0],1.0);
		    mcWPt_hist->Fill(events->mcWPt[0],1.0);
		    mcWEta_hist->Fill(events->mcWEta[0],1.0);
		    mcWPhi_hist->Fill(events->mcWPhi[0],1.0);
		    mcWCharge_hist->Fill(events->mcWCharge[0],1.0);
		    mcWMass_hist->Fill(events->mcWMass[0],1.0);
		    mcWNDaughters_hist->Fill(events->mcWNDaughters[0],1.0);
		    for (UInt_t k=0; k<events->mcWNDaughters[0]; k++) {
		      mcWDaughterId_hist->Fill(events->mcWDaughterId.at(0).at(k),1.0);
		      if(TMath::Abs(events->mcWDaughterId.at(0).at(k))==94) num_CMshowerFromW_++;
		      if(TMath::Abs(events->mcWDaughterId.at(0).at(k))==16) num_tauNeutrinoFromW_++;
		      if(TMath::Abs(events->mcWDaughterId.at(0).at(k))==15) num_tauFromW_++;
		      if(TMath::Abs(events->mcWDaughterId.at(0).at(k))==14) num_muonNeutrinoFromW_++;
		      if(TMath::Abs(events->mcWDaughterId.at(0).at(k))==13) num_muonFromW_++;
		      if(TMath::Abs(events->mcWDaughterId.at(0).at(k))==12) num_eleNeutrinoFromW_++;
		      if(TMath::Abs(events->mcWDaughterId.at(0).at(k))==11) num_eleFromW_++;
		      if(TMath::Abs(events->mcWDaughterId.at(0).at(k))==4) num_charmFromW_++;
		      if(TMath::Abs(events->mcWDaughterId.at(0).at(k))==3) num_strangeFromW_++;
		      if(TMath::Abs(events->mcWDaughterId.at(0).at(k))==2) num_upFromW_++;
		      if(TMath::Abs(events->mcWDaughterId.at(0).at(k))==1) num_downFromW_++;
		      if(TMath::Abs(events->mcWDaughterId.at(0).at(k))!=94 && TMath::Abs(events->mcWDaughterId.at(0).at(k))!=16  && TMath::Abs(events->mcWDaughterId.at(0).at(k))!=15  && TMath::Abs(events->mcWDaughterId.at(0).at(k))!=14  && TMath::Abs(events->mcWDaughterId.at(0).at(k))!=13   && TMath::Abs(events->mcWDaughterId.at(0).at(k))!=12   && TMath::Abs(events->mcWDaughterId.at(0).at(k))!=11  && TMath::Abs(events->mcWDaughterId.at(0).at(k))!=4  && TMath::Abs(events->mcWDaughterId.at(0).at(k))!=3  && TMath::Abs(events->mcWDaughterId.at(0).at(k))!=2  && TMath::Abs(events->mcWDaughterId.at(0).at(k))!=1)  num_otherFromW_++;
		    }
		    //}
		    ...
		    for (UInt_t j=0; j<events->mcMuon_N; j++) {
		      ...
		      if(events->mcMuonStatus[j]==1 && (TMath::Abs(events->mcMuonMotherId[j])==24 || TMath::Abs(events->mcMuonMotherId[j])==13)){
			num_goodMcMuons++;
		    ...
		    if(num_goodMcMuons!=1) cout<<"there are "<<num_goodMcMuons<<" MC muons with status==1 and muon or W as mother!!!"<<endl;
		    ...
		    */
		    ...
		    if ( (do_trigger_cut && (events->hltL2Mu20NoVertexNoBptx3BX || events->hltL2Mu30NoVertexNoBptx3BX || events->hltL2Mu20NoVertexNoBptx3BXNoHalo || events->hltL2Mu30NoVertexNoBptx3BXNoHalo || events->hltL2Mu20NoVertex2ChaNoBptx3BXNoHalo || events->hltL2Mu30NoVertex2ChaNoBptx3BXNoHalo ) ) || (!do_trigger_cut) ){ // pass trigger
		      ...
		      if(status1muonFromMuon || status1muonFromW) muStandAlonePt_FromW_hist->Fill(events->muStandAlonePt[0],1.0);
