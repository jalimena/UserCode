...
int main(int argc, char* argv[]){
  ...
  //if ((bx_cut=="with_bx_cut" || bx_cut=="without_bx_cut") && (cavern_cut=="with_cavern_cut" || cavern_cut=="without_cavern_cut") && (generatorMuon_cut=="with_generatorMuon_cut" || generatorMuon_cut=="without_generatorMuon_cut") && (trigger_cut=="with_trigger_cut" || trigger_cut=="without_trigger_cut") && (other_cut=="with_other_cut" || other_cut=="without_other_cut")) new_file = "/afs/cern.ch/user/j/jalimena/public/histos/stoppedHSCPMuonHistos_" + file_dataset + "_" + bx_cut + "_" + cavern_cut + "_" + generatorMuon_cut + "_" + other_cut + "_" + ".root";
  if ((bx_cut=="with_bx_cut" || bx_cut=="without_bx_cut") && (cavern_cut=="with_cavern_cut" || cavern_cut=="without_cavern_cut") && (generatorMuon_cut=="with_generatorMuon_cut" || generatorMuon_cut=="without_generatorMuon_cut") && (trigger_cut=="with_trigger_cut" || trigger_cut=="without_trigger_cut") && (other_cut=="with_other_cut" || other_cut=="without_other_cut")) new_file = "/home/alimena/histos/stoppedHSCPMuonHistos_" + file_dataset + "_" + bx_cut + "_" + cavern_cut + "_" + generatorMuon_cut + "_" + other_cut + "_" + ".root";
  ...
  TH1D* mcTopPt_hist = new TH1D("mcTopPt_hist","Gen Top p_{T}",1000,0,1000);
  ...
  TH1D* mcWPt_hist = new TH1D("mcWPt_hist","Gen W p_{T}",1000,0,1000);
  ...
  TH1D* mcMuonPt_hist = new TH1D("mcMuonPt_hist","Gen Muon p_{T}",1000,0,1000);
  ...
  TH1D* muStandAlonePt_hist = new TH1D("muStandAlonePt_hist","",1000,0,1000);
  ...
  TH2D* muStandAlonePt_muStandAloneTrackNChambersDt_hist = new TH2D("muStandAlonePt_muStandAloneTrackNChambersDt_hist","",1000,0,1000,10,0,10);
  ...
  TH1D* mu0_StandAlonePt_hist = new TH1D("mu0_StandAlonePt_hist","StandAlone Muon p_{T}",1000,0,1000);
  ...
  TH1D* mu1_StandAlonePt_hist = new TH1D("mu1_StandAlonePt_hist","StandAlone Muon p_{T}",1000,0,1000);
  ...
  int pass_Nstaus_cut = 0;
  int pass_tauFromStau_cut = 0;
  int pass_muon_cut = 0;
  ...
  for (Int_t i=0; i<nentries; i++) {
	...
	if( (do_generatorMuon_cut && events->mcStau_N>0) || (!do_generatorMuon_cut)){
	  pass_Nstaus_cut++;
	    if( (do_generatorMuon_cut && tauFromStau)  || (!do_generatorMuon_cut)){
		...
		if( (do_generatorMuon_cut && status1muon) || (!do_generatorMuon_cut)){
		  ...
		  if( (do_generatorMuon_cut && status1muonFromTau) || (!do_generatorMuon_cut)){
		    ...
		    //cout<<"passed mcStoppedParticle"<<endl;
		    ...
		    */
                    ...
                    for (UInt_t j=0; j<events->mcMuon_N; j++) {
                      ...
                      if(events->mcMuonStatus[j]==1 && TMath::Abs(events->mcMuonMotherId[j])==15){
                        num_goodMcMuons++;
	    ...
	    } //end of muon from stau cuts
	} //end of num of staus cut
...
*/
  ...
  cout<<"number of events passing number of staus cut is: "<<pass_Nstaus_cut<<endl;
  cout<<"number of events passing tau from Stau cut is: "<<pass_tauFromStau_cut<<endl;
  ...
  cout<<"number of events passing status1 muon from stau (generator muon) cut is: "<<pass_generatorMuon_cut<<endl;
