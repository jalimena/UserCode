...
set dataset=$argv[1]
mkdir $dataset'_separateEvents_725'
cd $dataset'_separateEvents_725'
...
if ($host_ == 'brux') then #brux6 is host
    find '/mnt/hadoop/store/user/jalimena/Ntuples/'$dataset'_separateEvents_particle'*'_725/stoppedHSCPMuonTree.root' -type f > list.txt
endif
...
if ($host_ == 'brux') then #brux6 is host
    lcg-cp -b -v -D srmv2 file:./stoppedHSCPMuonTree.root srm://srm.hep.brown.edu:8443/srm/v2/server'?''SFN=/mnt/hadoop/store/user/jalimena/Ntuples/'$dataset'_separateEvents_725/stoppedHSCPMuonTree.root' > & lcg-cp.out
endif
if ($host_ == 'lxpl') then #lxplus is host
